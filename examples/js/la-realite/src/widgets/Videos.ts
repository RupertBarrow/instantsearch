...
type Hit = {
  // CHANNEL
  // TODO : get this from yt-dlp = channel url (to replace avatar_url) and owner (to replace job_title)
  channel?: {
    id: string;
    title: string;
...
// const getBlogPostUrl = (hit: Hit) =`https://algolia.com/blog/${hit.primary_category.slug}/${hit.slug}`;
const getBlogPostUrl = (hit: Hit) => hit.webpage_url;
...
function createHit(
  hit: Hit,
  {
    ...
    refinedCategory: string | undefined;
  }
) {
  ...
  const primaryCategory = hit.primary_category
    ? `<span class="card-subject">
         ${refinedCategory || hit.primary_category.title}
       </span> • `
    : '';
  const cardTitle = highlight({ attribute: 'video.title', hit });
  const date = formatDistanceToNow(hit.video.publishedDate.timestamp, { addSuffix: true }).replace('about ', '');
  const cardDescription = hit.caption.content;
  const footerChannel = channel
    ? `<div class="card-author"><img class="card-author-avatar" src="${channel.avatar_url}" alt="${channel.title}" /><span class="card-author-name">${channel.title}<span class="card-author-job">${channel.job_title}</span></span></div>`
    : '';
  const cardGg = highlight({
    attribute: 'video.title',
    hit,
  });

  return `
    <li class="ais-InfiniteHits-item${isHighlighted ? ' infinite-hits-item--highlighted' : ''}">
      <a class="card-link" href="${getBlogPostUrl(hit)}">
        <article class="card">
          <div class="card-image">
            <img src="${hit.video.thumbnail}" alt="${hit.video.title}" />
          </div>
...
const infiniteHits = connectInfiniteHits<{ container: string }>(
  (
      ...
      containerNode.querySelector('div')!.innerHTML = `
        <div class="infinite-hits-no-results-container">
          ...
          <p class="infinite-hits-no-results-paragraph">
            Désolé, nous ne trouvons pas de réponse${results.query ? ` pour "${results.query}"` : ''}.
          </p>
        </div>
      `;
    ...
    const refinedCategory = ((facet) => {
      const category = facet && facet.data && facet.data.find(({ isRefined }) => isRefined);
    ...
    containerNode.querySelector('div')!.innerHTML = `
      <div class="previous-hits">
        <p class="previous-hits-message">
          ...
          ${results.nbHits} vidéos
        </p>
        <button class="previous-hits-button">Vidéo précédente</button>
      </div>
      <ol class="ais-InfiniteHits-list">
        ${hits.map((hit, index: number) =>
      createHit(hit as unknown as Hit, {
        isHighlighted: results!.nbHits !== 3 && (index === 0 || results!.nbHits === 2),
        refinedCategory,
      })
    ).join('')}
      </ol>
      ...
      ${results.nbHits > 0 && isLastPage
        ? `<div class="infinite-hits-end">
            <p>${results.nbHits} vidéos listées</p>
          </div>`
        : ''
      }
    `;
